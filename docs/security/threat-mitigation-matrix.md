...
| A3 | Network MITM | inject/replay/downgrade | WSS + mTLS; DTLS-SRTP; AEAD everywhere; replay dedup by eid; domain-bound auth challenge | T02 auth-replay test; T05 TURN ciphertext capture check; 1.32 relay replay cell (a duplicate is never accepted twice) · **OPEN DEFECT — [task 2.13](../tasks/phase-2/2.13-ratchet-replay-dos.md):** a single replayed envelope **permanently wedges the receiving ratchet chain**. `Ratchet::decrypt` (`apps/crypto/src/ratchet.rs`) advances `ckr`/`nr` *before* `aead_open` and does not roll them back on failure, so after one duplicate every subsequent genuine message from that sender fails. Unauthenticated, key-material-free, permanent session DoS mountable by any relay (A2/A3), and it breaks benign duplicate delivery too. Confidentiality/authenticity are unaffected. Measured in `apps/cli/tests/relay_attacks.rs::replayed_delivery_is_not_accepted_twice`; deliberately **not** asserted there, since asserting today's behaviour would entrench it. Contradicts threat-model goal 6 ("availability degradation is honest but not silent") — so it is a defect to fix, not a residual to accept, unless security-reviewer signs off otherwise |
