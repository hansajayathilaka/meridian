...
4. Run the task's Tests (narrowest first). Get the required **Reviews** sign-off (security-reviewer / architect / test-engineer / code-reviewer as the task specifies) — this single pass **is** the security/architecture gate in the [Definition of Done](../../CONTRIBUTING.md); do not also run `/review` on the same diff, that command is for ad-hoc scopes outside this workflow. Satisfy the remaining Definition of Done gates; run `/doc-sync` if behaviour/wire/diagrams changed.
