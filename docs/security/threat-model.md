...
4. **Deniability (weak) — met by envelope v2, scoped precisely.** Message authentication is by MACs within ratchet sessions, with identity-key signatures confined to key distribution and signaling (standard Signal-style deniability): **envelope v2** drops the per-message identity-key signature ([ADR 0016](../adr/0016-envelope-deniability.md)), so message authentication reduces to the ratchet AEAD under keys the recipient also holds — no v2 ciphertext is third-party-verifiable as any particular sender's, and any transcript can be forged by the recipient. This covers *authorship* only, not *participation*: bundle signatures, the domain-bound rendezvous auth signature, and account-signed device records remain identity-key signatures proving a key was live at a given server at a given time (ADR 0016 residual R4). It is also **single-hop only** — the federated case is one hop further and is not fully resolved by ADR 0016 alone (see [ADR 0017](../adr/0017-federation-trust-boundary.md)). This is weak, Signal-grade deniability, not OTR-/court-grade, and there is no online/interactive deniability. (The now-superseded envelope v1 signed every ratchet ciphertext with the sender's identity key and had no deniability property at all; v1 no longer ships.) Do not claim deniability beyond this precise scope in code, copy, or docs.
...
- **Deniability scope (goal 4, now met for envelope v2, authorship only, single-hop).** [ADR 0016](../adr/0016-envelope-deniability.md) removed the per-message identity-key signature at envelope v2, so authorship of v2 messages is no longer third-party-provable, and the offline mailbox ([ADR 0007](../adr/0007-offline-mailbox.md)) now stores unattributable bytes rather than signed ciphertexts. This is true single-hop only — the federated case is one hop further and is not fully resolved by ADR 0016 alone (see [ADR 0017](../adr/0017-federation-trust-boundary.md)). Deniability of *participation* (bundle/auth/device signatures) remains out of scope in every version. The now-superseded envelope v1 signed every ratchet ciphertext with the sender's identity key and had no deniability property at all; v1 no longer ships.
