...
4. **Deniability (weak) — a target, NOT a property of envelope v1.** *As shipped today this goal is unmet:* [envelope v1](../api/messaging-envelope-v1.md) signs every ratchet ciphertext with the sender's Ed25519 identity key, so v1 transcripts **are** third-party-provable as to authorship. The intended property — message authentication by MACs within ratchet sessions, with identity-key signatures confined to key distribution and signaling (standard Signal-style deniability) — is realized at **envelope v2**, which drops the per-message signature ([ADR 0016](../adr/0016-envelope-deniability.md)). Even then it covers *authorship*, not *participation*: bundle signatures, the rendezvous auth signature, and device records remain identity-key signatures (see ADR 0016 residual R4). Do not claim deniability for v1 traffic in code, copy, or docs.
...
- **Deniability of envelope-v1 traffic (goal 4, currently unmet).** Every v1 envelope carries an identity-key signature over the ratchet ciphertext, so authorship of v1 messages is third-party-provable — and the offline mailbox ([ADR 0007](../adr/0007-offline-mailbox.md)) stores those signed ciphertexts server-side for its TTL. [ADR 0016](../adr/0016-envelope-deniability.md) decides to remove the signature at envelope v2; until that lands, treat v1 transcripts as attributable. Deniability of *participation* (bundle/auth/device signatures) is out of scope in every version.
