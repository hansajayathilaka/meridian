...
## Acceptance criteria
All four NAT matrix cells connect (symmetric×symmetric via relay; `udp-blocked` + `relay-only` excepted — proven impossible at the pinned dependency version, see [1.30](../../tasks/phase-1/1.30-turn-tcp-dependency-gap.md)); TLS-443 fallback works with UDP fully dropped; credentials expire and are distinct per request (reuse of a captured credential within its TTL is bounded by coturn's `user-quota`, not rejected outright); in `relay-only`, a packet capture at the peer contains zero of our host/srflx addresses; TURN sees only DTLS ciphertext. **Closed at the wire level by [1.27](../../tasks/phase-1/1.27-pcap-assertions-ci.md):** `tools/netns-nat-matrix.sh`'s pcap-analysis assertions now prove the last two clauses against real tcpdump captures (not just simulation/in-process) for full-cone/port-restricted/symmetric:symmetric, plus a udp-blocked-specific fails-fast wire assertion; run via `bash harnesses/nat-matrix/run.sh` when root/`NET_ADMIN` is available (skips cleanly otherwise — see that script and `tools/netns-nat-matrix.sh`'s `need_root()`), so "capture inspected in CI" is conditional on the CI runner actually having `NET_ADMIN`, not an unconditional CI guarantee.
