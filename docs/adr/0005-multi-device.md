...
**Status:** Accepted · **Options:** (A) copy the identity key to every device; (B) **per-device subkeys signed by the account key, Sesame-style per-device sessions (chosen)**; (C) each device is a first-class independent identity (CoMediation/"device = contact").
