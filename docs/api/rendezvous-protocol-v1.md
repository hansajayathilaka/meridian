...
TOML; every field has a default (see [`meridian-rendezvous` `config`](../../apps/rendezvous/src/config.rs)). Loading goes through [`figment`](https://docs.rs/figment) (ADR [0018](../adr/0018-rendezvous-config-loading.md)): `Config::load` merges the TOML file with `MERIDIAN_<SECTION>__<FIELD>` environment variables (e.g. `[turn] secret` → `MERIDIAN_TURN__SECRET`), env taking precedence key-by-key — see the per-key env names in [`rendezvous.example.toml`](../../apps/rendezvous/rendezvous.example.toml). This is the intended way to inject secrets (`[turn].secret`) and per-deployment overrides into a container/Helm deploy without templating the TOML file itself. Lists (`invite_tokens`, `turn.urls`) use figment's native TOML/JSON bracket syntax, e.g. `MERIDIAN_TURN__URLS=["turn:a","turn:b"]` — not comma-separated.
