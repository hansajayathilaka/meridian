...
| [0022](./0022-release-binary-distribution.md) | Rolling `rendezvous-latest` GitHub Release with Linux + Windows `x86_64` executables, built on the same trigger as the Docker image (Accepted); signing deferred, same trigger as 0019's residual | executable-pipeline task |
