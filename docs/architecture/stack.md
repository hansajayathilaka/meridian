...
| **Crypto** | Rust | X3DH + Double Ratchet (libsignal *or* assembled from vodozemac + RustCrypto — see ADR-R3); OpenMLS for groups (Phase 3); ML-KEM via `ml-kem`/`pqcrypto` for the PQXDH slot | never bespoke; audited libraries only |
...
**Core / crypto:** `tokio`, `serde` + `ciborium` (CBOR, DOC-01), `ed25519-dalek` + `x25519-dalek` (identity/DH), `hkdf` + `sha2`/`sha2-512` (KDF, safety numbers), `chacha20poly1305` + `aes-gcm` (AEAD), `blake3` (file merkle, content addressing — T09/T15), `zeroize` (key hygiene). Ratchet: **libsignal-client** *or* **vodozemac** + hand-wired X3DH (ADR-R3). Groups (Phase 3): **openmls**. PQ slot (Phase 2): **ml-kem**.
...
**Trade-offs:** C reintroduces bespoke-crypto risk — rejected per the design's core principle. A is the most battle-tested implementation on earth but carries **AGPL-3.0** licensing and an API oriented to Signal's own app, which complicates embedding in a differently-licensed, self-hostable product. B is Apache-2.0, audited, WASM-friendly, and closer to a library (built for reuse), but covers the ratchet — we still wire X3DH prekey handling around it. **Decision: default to B (vodozemac + thin X3DH), with A evaluated in a Phase-0 spike** measured on: WASM/aarch64 build cleanliness, header-encryption support, and the license fit for self-hosting. `cargo-deny` gates the license decision in CI. **Consequence:** whichever wins, it sits behind `meridian-crypto`'s API so the rest of the tree is insulated; the choice is reversible without touching `meridian-core` consumers.
...
2. **libsignal vs vodozemac** (ADR-R3) — resolved by the Phase-0 spike + `cargo-deny` license gate, not by preference.
