...
| A4 | Compromised peer | learn your IP, other contacts, escalate | relay-only policy hides IP (§5.4); no contact-graph on wire; per-contact tunnel allowlist (T16); grant expiry (T15) | T05 relay-only packet capture; T16 allowlist bypass tests |
| A5 | Metadata observer | who-talks-to-whom, timing, IPs via ICE | org-bounded metadata (ADR-2); relay-only; header encryption; mailbox padding (Phase 3) | documented residual (§1.3); T05 IP-leak capture; **open: mixnet (§12 Q1)** |
