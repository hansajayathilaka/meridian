...
**Impersonation.** Nothing a server does can make a client accept the wrong key: sessions are addressed *to keys*, bundles are verified *against keys*, and envelopes are authenticated *by keys* (v1: an identity-key signature over the ciphertext; v2: the ratchet AEAD bound to both identity keys — [ADR 0016](../adr/0016-envelope-deniability.md)). The residual risk is human-layer: an attacker hands you an ID and claims to be "Dana from finance." Mitigations: safety-number verification (§4.4) with QR flow for in-person/video verification; TOFU pinning with loud, blocking alerts on key change; org-level **directory attestations** — an enterprise MAY run an internal signed directory mapping HR identities → account keys, which clients treat as a *petname source with provenance*, not as a key authority (a wrong directory entry is detectable by the victim, whose client knows its own key).
...
- **Stream open:** `OPEN{stream_id, type:"mrd.file/1", params, channel_config}` → peer's policy layer accepts/rejects (auto-accept for chat, prompt for screenshare/SSH, org policy hooks). On accept, **both sides** open a new SCTP data channel labeled `"{type}#{stream_id}"` — the responder calls `add_data_channel` when it decides to accept (alongside sending `Accept`), and the initiator calls it symmetrically on receiving that `Accept`, since a negotiated data channel fires no `ondatachannel`-equivalent event on a side that never opened it itself. Channel config per type: reliable/ordered for chat & control; reliable/unordered for file chunks; unreliable/unordered (maxRetransmits=0) for live location and game-like streams; media stream types attach RTP transceivers instead of data channels.
- **Framing:** every stream frame is ratchet-sealed directly, one Double Ratchet step per frame call (not a separately-derived per-stream key set up once, as an earlier draft of this section described) — SCTP already frames messages, so there is no length prefix on the wire. Each call also derives a one-way HKDF export of the message key it just consumed (`info` = stream type ‖ stream id), computed and immediately zeroized on both ends; no stream type consumes it today (a plausible future one: a high-rate media-adjacent stream wanting fast symmetric AEAD instead of a full ratchet step per packet). Full mechanism and wire shape: `docs/api/stream-types-v1.md`'s "Stream framing" section, the authority for this detail.
...
Sender: generate per-file key `k_f`; chunk (64 KiB); build BLAKE3 merkle tree; send `mrd.file/1` OPEN on ctrl with manifest `{name, size, root, enc(k_f under ratchet)}` → recipient policy accepts → sender streams AEAD-sealed chunks (key `k_f`, nonce = chunk index) on a reliable-unordered channel, throttled by `bufferedAmount` watermarks; recipient writes by offset, verifies subtrees incrementally; on disconnect, session redial + recipient sends missing-range bitmap → resume (in-stream, not a ctrl frame — see `docs/api/stream-types-v1.md`'s `mrd.file/1` section; no per-range ack exists outside that resume bitmap). Integrity = merkle root check; the per-file key means the *same ciphertext* can later be offered to other authorized peers (dedup/reshare) without re-encryption while remaining bound to an explicit key grant per recipient. **Status:** this reshare/dedup path is design-permitted, not yet built — every `mrd.file/1` transfer shipped so far generates and seals an independent fresh `k_f` even for a file already sent once; the feature spec records this explicitly as an out-of-scope follow-up (`docs/architecture/features/09-file-transfer.md`), tracked as an open, unowned carry-forward ([docs/tasks/README.md](../tasks/README.md#live-carry-forwards-not-owned-by-any-open-task)).
