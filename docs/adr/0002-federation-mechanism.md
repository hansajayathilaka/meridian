...
**Status:** Accepted · **Options:** (A) Kademlia-style DHT for discovery + hole-punching relays; (B) **federated s2s signaling, DNS-SRV/static-map discovery (chosen)**; (C) rendezvous hashing across a shared server list; (D) gossip mesh among servers.
