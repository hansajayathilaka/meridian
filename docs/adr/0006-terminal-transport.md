...
**Status:** Accepted · **Options:** (A) headless Chromium; (B) **native WebRTC library — webrtc-rs (Rust) or libdatachannel (C++) (chosen)**; (C) QUIC/WebTransport between non-browser peers with WebRTC only when a browser participates; (D) plain TLS-TCP with manual traversal.
