...
| A4 | Compromised peer | learn your IP, other contacts, escalate | relay-only policy hides IP (§5.4); no contact-graph on wire; per-contact tunnel allowlist (T16); grant expiry (T15) | 1.16 observed-candidate enforcement (code-level, fail-closed abort — done); T05/1.23 relay-only packet capture (wire-level — pending); T16 allowlist bypass tests |
| A5 | Metadata observer | who-talks-to-whom, timing, IPs via ICE | org-bounded metadata (ADR-2); relay-only; header encryption; mailbox padding (Phase 3) | documented residual (§1.3); 1.16 observed-candidate enforcement (code-level — done); T05/1.23 IP-leak packet capture (wire-level — pending); **open: mixnet (§12 Q1)** |
