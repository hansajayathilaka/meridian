...
| A2 | Malicious signaling server | key substitution MITM; drop/forge | addressed-to-key fetch + sig verify (§3.3); fp-in-envelope (§4.6); verified⇒block on change (§4.4); per-message envelope signature (**removed at v2**, per [ADR 0016](../adr/0016-envelope-deniability.md) — v2 envelopes carry no signature at all; the row does not depend on one, since bundle-sig verification, the ratchet AEAD's identity binding (fails closed on tamper), and block-on-change carry it) | T02 tampered-bundle test; 1.28 active relay-rewrite of routed blobs (detected by the ratchet AEAD, fails closed); 1.32 relay attacks that reach the recipient because a routing-only relay never touches AEAD-authenticated bytes — forged `Deliver.from`, replay, reorder, drop, cross-delivery — plus X3DH preamble mutation (each rejected with its own diagnosable error, consuming no prekey and installing no session, per ADR 0016 C2's commit-on-successful-decrypt); T08 `meridian-mitm-sim` matrix (0 silent wins **outside the enumerated accepted residuals** — enumerated in [ADR 0016](../adr/0016-envelope-deniability.md) or §1.3 only, not "any ADR"; the exception is **not live under the now-superseded envelope v1**, where the requirement was 0 silent wins unqualified, and extending the list needs a new ADR with security-reviewer sign-off); [task 2.13](../tasks/phase-2/2.13-ratchet-replay-dos.md) fixed the one case that used to survive a replay only at the cost of wedging the session — see A3 |
...
- OTK depletion bounded per-source; signed-prekey fallback weakens only first-message deniability, never confidentiality (T02). This is now a **live** consideration under envelope v2, which achieves message deniability ([ADR 0016](../adr/0016-envelope-deniability.md)); it was moot under the now-superseded envelope v1, which had no deniability property at all regardless — every v1 ciphertext was identity-signed.
